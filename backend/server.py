...
async def chat_assistant(chat: ChatMessage, current_user: User = Depends(get_current_user)):
    ...
    events = await db.events.find({"user_id": current_user.id}, {"_id": 0}).to_list(20)
    
    clients_list = chr(10).join([f"- ID: {c['id']}, Nome: {c['name']}, Tel: {c.get('phone','')}" for c in clients]) or "Nenhum"
    events_list = chr(10).join([f"- ID: {e['id']}, Tipo: {e['event_type']}, Data: {e['event_date']}" for e in events[:5]]) or "Nenhum"
    today = date.today().strftime("%d/%m/%Y")
    current_year = date.today().year

    # Parse simples por palavras-chave como fallback
    msg = user_message.lower()
    
    if any(w in msg for w in ["listar cliente", "ver cliente", "meus clientes", "lista de clientes"]):
        if clients:
            lista = chr(10).join([f"• {c['name']}{' - ' + c['phone'] if c.get('phone') else ''}" for c in clients])
            return {"action": "list_clients", "message": f"Seus clientes:{chr(10)}{lista}"}
        return {"action": "answer", "message": "Nenhum cliente cadastrado ainda."}
    
    if any(w in msg for w in ["listar evento", "ver evento", "meus eventos", "lista de eventos"]):
        if events:
            lista = chr(10).join([f"• {e['event_type']} - {e['event_date'][:10]}" for e in events[:10]])
            return {"action": "list_events", "message": f"Seus eventos:{chr(10)}{lista}"}
        return {"action": "answer", "message": "Nenhum evento cadastrado ainda."}

    # Tentar IA
    system_prompt = f"""Você é o assistente do FOTIVA para fotógrafos.
Data: {today} | Ano: {current_year}
Clientes: {clients_list}
Eventos: {events_list}

Interprete linguagem natural. Se cliente nao existe, use create_both.
RESPONDA SOMENTE JSON:
{{"action":"create_event|create_client|create_both|answer","message":"texto","client":{{"name":"","phone":null,"email":null}},"event":{{"client_name":"","client_id":null,"event_type":"","event_date":"YYYY-MM-DDTHH:MM:00","location":"","total_value":0,"amount_paid":0,"notes":""}}}}"""

    raw = None
    try:
        import anthropic as ant
        ai = ant.Anthropic(api_key=os.environ.get("ANTHROPIC_API_KEY",""))
        resp = ai.messages.create(model="claude-haiku-4-5-20251001", max_tokens=500,
            system=system_prompt, messages=[{"role":"user","content":user_message}])
        raw = resp.content[0].text
    except Exception as ex:
        logger.warning(f"IA indisponivel: {ex}")

    if not raw:
        return {"action":"answer","message":f"Diga por exemplo: 'Criar evento para João, casamento dia 15/03, valor 2500'"}

    try:
        clean = re.sub(r"```json|```","",raw).strip()
        parsed = json.loads(clean)
    except:
        return {"action":"answer","message":"Nao entendi. Tente: 'Criar evento para [nome], [tipo] dia [data], valor [R$]'"}

    action = parsed.get("action","answer")

    if action == "create_client":
        cd = parsed.get("client",{})
        if cd.get("name"):
            nc = {"id":str(uuid.uuid4()),"user_id":current_user.id,"name":cd["name"],
                "phone":cd.get("phone") or "","email":cd.get("email") or "","notes":"",
                "created_at":datetime.now(timezone.utc).isoformat()}
            await db.clients.insert_one(nc)
            parsed["message"] = f"Cliente {cd['name']} criado com sucesso!"

    elif action in ("create_both","create_event_and_client"):
        cd = parsed.get("client",{})
        ed = parsed.get("event",{})
        cid = None
        if cd.get("name"):
            ...
            await db.clients.insert_one(nc)
            cid = nc["id"]
        if cid and ed.get("event_type"):
            ne = {"id":str(uuid.uuid4()),"user_id":current_user.id,"client_id":cid,
                "event_type":ed.get("event_type","Evento"),"event_date":ed.get("event_date",datetime.now().isoformat()),
                "location":ed.get("location",""),"total_value":float(ed.get("total_value",0)),
                "amount_paid":float(ed.get("amount_paid",0)),"remaining_installments":1,
                "notes":ed.get("notes",""),"status":"confirmado","created_at":datetime.now(timezone.utc)}
            await db.events.insert_one(ne)
            parsed["message"] = f"Cliente {cd.get('name','')} e evento {ed.get('event_type','')} criados!"

    elif action == "create_event":
        ed = parsed.get("event",{})
        cid = ed.get("client_id")
        if not cid:
            cname = ed.get("client_name","")
            found = next((c for c in clients if cname.lower() in c["name"].lower()),None)
        ...
        if cid:
            ne = {"id":str(uuid.uuid4()),"user_id":current_user.id,"client_id":cid,
                ...
                "notes":ed.get("notes",""),"status":"confirmado","created_at":datetime.now(timezone.utc)}
            await db.events.insert_one(ne)
            parsed["message"] = f"Evento {ed.get('event_type','')} criado com sucesso!"
        else:
            parsed["action"] = "ask"
            parsed["message"] = f"Cliente nao encontrado. Devo criar o cliente tambem?"
