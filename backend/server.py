...
async def get_events(current_user: User = Depends(get_current_user)):
    ...
    for event in events:
        ...
        # Buscar nome do cliente se não estiver no evento
        if not event.get('client_name') and event.get('client_id'):
            client = await db.clients.find_one({"id": event['client_id'], "user_id": current_user.id}, {"_id": 0})
            if client:
                event['client_name'] = client.get('name', '')
    return events
...
@api_router.post("/payments/sync-from-events")
async def sync_payments_from_events(current_user: User = Depends(get_current_user)):
    """Gera automaticamente cobranças baseadas no amount_paid dos eventos"""
    events = await db.events.find({"user_id": current_user.id}, {"_id": 0}).to_list(1000)
    created = 0
    ...
    for event in events:
        ...
        remaining = total_value - amount_paid
        
        # Buscar nome do cliente
        client_name = event.get('client_name', '')
        if not client_name and event.get('client_id'):
            client = await db.clients.find_one({"id": event['client_id'], "user_id": current_user.id}, {"_id": 0})
            if client:
                client_name = client.get('name', '')
        
        # Verificar se já existem pagamentos para este evento
        existing = await db.payments.find({"event_id": event_id, "user_id": current_user.id}, {"_id": 0}).to_list(100)
        
        if not existing:
            # Criar pagamento do valor já pago (se houver)
            if amount_paid > 0:
                paid_payment = {
                    "id": str(uuid.uuid4()),
                    ...
                    "total_installments": 2 if remaining > 0 else 1,
                    "amount": amount_paid,
                ...
                await db.payments.insert_one(paid_payment)
                created += 1
            
            # Criar pagamento do valor restante (se houver)
            if remaining > 0:
                remaining_payment = {
                    "id": str(uuid.uuid4()),
                    "user_id": current_user.id,
                    ...
                    "client_name": client_name,
                    "event_name": event.get('event_type', ''),
                    "installment_number": 2 if amount_paid > 0 else 1,
                    "total_installments": 2 if amount_paid > 0 else 1,
                    "amount": remaining,
                    "due_date": event.get('event_date', datetime.now().strftime('%Y-%m-%d'))[:10],
                    "paid": False,
                    "paid_date": None,
                    "payment_link": None,
                    "created_at": datetime.now(timezone.utc).isoformat()
                }
                await db.payments.insert_one(remaining_payment)
                created += 1
    
    return {"message": f"{created} cobranças criadas com sucesso", "created": created}
