...
def extrair_data(texto):
    ...
    if m:
        ...
        if ano < 100: ano += 2000
        hora = '09:00'
        h = re.search(r'(\d{1,2})h(\d{2})?', t)
        if h: hora = f"{int(h.group(1)):02d}:{h.group(2) or '00'}"
        try: return f'{ano:04d}-{mes:02d}-{dia:02d}T{hora}:00'
        except: pass
    
    meses = {'janeiro':1,'fevereiro':2,'marco':3,'marco':3,'abril':4,'maio':5,'junho':6,
             'julho':7,'agosto':8,'setembro':9,'outubro':10,'novembro':11,'dezembro':12}
    for nome, num in meses.items():
        m = re.search(rf'(\d{{1,2}})\s*(?:de\s*)?{nome}(?:\s*(?:de\s*)?(\d{{4}}))?', t)
        if m:
            dia = int(m.group(1))
            ano = int(m.group(2)) if m.group(2) else hoje.year
            hora = '09:00'
            h = re.search(r'(\d{1,2})h(\d{2})?|as\s+(\d{1,2}):(\d{2})', t)
            if h:
                if h.group(1): hora = f"{int(h.group(1)):02d}:{h.group(2) or '00'}"
                elif h.group(3): hora = f"{int(h.group(3)):02d}:{h.group(4)}"
            return f'{ano:04d}-{num:02d}-{dia:02d}T{hora}:00'
    return None
...
def extrair_nome_cliente(texto_original):
    import re
    padroes = [
        r'para\s+(?:o\s+|a\s+)?([A-Za-zÀ-ú]+(?:\s+[A-Za-zÀ-ú]+){0,3})\s*(?:,|casamento|aniversar|ensaio|evento|formatura|infantil|dia\s+\d|valor)',
        r'para\s+(?:o\s+|a\s+)?([A-Za-zÀ-ú]+(?:\s+[A-Za-zÀ-ú]+){0,3})',
        r'cliente\s+([A-Za-zÀ-ú]+(?:\s+[A-Za-zÀ-ú]+){0,3})',
    ]
    for p in padroes:
        m = re.search(p, texto_original, re.IGNORECASE)
        if m:
            nome = m.group(1).strip().rstrip(',')
            if len(nome) > 2 and nome.lower() not in ['um','uma','novo','nova','o','a']:
                return nome
    return None
...
async def chat_assistant(chat: ChatMessage, current_user: User = Depends(get_current_user)):
    ...
    if any(w in t for w in ['novo cliente','criar cliente','adicionar cliente','cadastrar cliente']):
        ...
        nc = {"id":str(uuid.uuid4()),"user_id":current_user.id,"name":nome,"phone":telefone,
              "email":"","notes":"","created_at":dt.now(timezone.utc).isoformat()}
    ...
    # CRIAR EVENTO
    tem_tipo = any(w in t for w in ['casamento','aniversar','ensaio','formatura','debutante','infantil','batizado','corporativo','festa'])
    tem_criar = any(w in t for w in ['criar','novo evento','agendar','cadastrar evento','marcar'])
    
    if tem_criar or tem_tipo:
        ...
        if not cliente:
            nc = {"id":str(uuid.uuid4()),"user_id":current_user.id,"name":nome_cliente,
                  "phone":"","email":"","notes":"","created_at":dt.now(timezone.utc).isoformat()}
            await db.clients.insert_one(nc)
            cliente = nc
