...
try:
    ...
    print("⚠️ pywebpush não instalado - notificações push desativadas")
...
async def update_client(client_id: str, client_data: ClientCreate, current_user: User = Depends(get_current_user)):
    ...
    if client_data.phone is not None:
        update_data['phone'] = client_data.phone if client_data.phone != '' else None
    if client_data.email is not None:
        update_data['email'] = client_data.email if client_data.email != '' else None
...
VAPID_EMAIL = os.environ.get('VAPID_EMAIL', 'mailto:admin@fotiva.com')

@api_router.get("/push/vapid-public-key")
async def get_vapid_public_key():
    if not VAPID_PUBLIC_KEY:
        raise HTTPException(status_code=500, detail="VAPID não configurado")
...
async def send_push_to_user(user_id: str, title: str, body: str, url: str = "/"):
    ...
    if not WEBPUSH_AVAILABLE or not VAPID_PRIVATE_KEY:
        print("Push não disponível - WEBPUSH ou VAPID não configurado")
        return False
    try:
...
async def test_push_notification(current_user: User = Depends(get_current_user)):
    ...
    if success:
        ...
        raise HTTPException(status_code=400, detail="Erro ao enviar notificação. Verifique se as notificações estão ativadas.")

@api_router.post("/push/notify-events")
async def notify_upcoming_events(background_tasks: BackgroundTasks, current_user: User = Depends(get_current_user)):
    """Verifica eventos próximos e envia notificações"""
    background_tasks.add_task(check_and_notify_events, current_user.id)
    return {"message": "Verificando eventos..."}

async def check_and_notify_events(user_id: str):
    """Verifica eventos nas próximas 48h e envia notificações"""
    try:
        now = datetime.now(timezone.utc)
        events = await db.events.find({"user_id": user_id}).to_list(100)
        for event in events:
            ...
            try:
                ...
                data_fmt = event_date.strftime("%d/%m às %H:%M")
                if 47 <= hours <= 49:
                    await send_push_to_user(user_id, f"📅 Lembrete 48h - {event_type}", f"Amanhã: {event_type} de {client_name} - {data_fmt}")
                elif 23 <= hours <= 25:
                    await send_push_to_user(user_id, f"⏰ Lembrete 24h - {event_type}", f"Amanhã: {event_type} de {client_name} - {data_fmt}")
                elif 11 <= hours <= 13:
                    await send_push_to_user(user_id, f"🔔 Lembrete 12h - {event_type}", f"Hoje: {event_type} de {client_name} - {data_fmt}")
            except Exception as e:
                print(f"Erro ao processar evento {event.get('id')}: {e}")
    except Exception as e:
        print(f"Erro ao verificar eventos: {e}")

@api_router.get("/health")
async def health_check():
    return {"status": "ok", "push_available": WEBPUSH_AVAILABLE, "vapid_configured": bool(VAPID_PUBLIC_KEY)}
